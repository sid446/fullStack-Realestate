...
export default function SignUp() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setLoading(false);
      setError(null);
      navigate('/sign-in');// if every thing is fine we wil navigate to sign in 
    } catch (error) {
      setLoading(false);
    ...
    >
      <div className="flex flex-col gap-6 justify-center items-center h-[35rem] w-[30rem] bg-slate-100 bg-opacity-50">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          <button disabled={loading} className="w-[20rem] rounded-full h-[3rem] hover:scale-105 transform transition-transform duration-200 ease-in-out hover:bg-zinc-700 hover:text-lg hover:text-white font-semibold bg-white">
